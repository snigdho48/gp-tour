...
function TripDownloadOptions({ option, people }) {
  ...
  const downloadTripImage = () => {
    ...
    logoImg.onload = () => {
      ...
      const logoSize = 60;
      const textWidth = 120; // Approximate width of "GPStar" text
      ...
      // Draw logo with the specified color
      ctx.save();
      ...
      ctx.textAlign = "left";
      ctx.fillText("GPStar Tour Planner", logoX + logoSize + 20, logoY + 35);

      // Tagline below the logo-text row
    ...
    const textWidth = 120; // Approximate width of "GPStar" text
    const totalWidth = logoSize + 20 + textWidth; // Logo + spacing + text
    const startX = (canvas.width - totalWidth-80) / 2;
    ...
    ctx.font = "bold 24px TelenorEvolution, sans-serif";
    ctx.textAlign = "center";
    ctx.fillText("GPStar Tour Planner", canvas.width / 2 + 50, 65);
    ...
    const breakdownItems = [
      {
        label: "Flight",
        amount: breakdown.flight,
        color: "#3b82f6",
        percentage: Math.round((breakdown.flight / breakdown.total) * 100),
      },
      {
        label: "Hotel",
        amount: breakdown.hotel,
        color: "#10b981",
        percentage: Math.round((breakdown.hotel / breakdown.total) * 100),
      },
      {
        label: "Activities",
      ...
      },
      {
        ...
        color: "#ef4444",
        percentage: Math.round((breakdown.contingency / breakdown.total) * 100),
      },
    ];
    ...
    if (option.type === "International") {
      ...
      ctx.font = "18px Arial, sans-serif";
      ctx.fillText(
        "Get GPStar roaming packages for your trip to stay",
        canvas.width / 2,
        yPosition + 60
      );
      ctx.fillText(
        "connected with family and share your travel moments!",
        canvas.width / 2,
        yPosition + 85
      );
  ...
  const downloadTripPDF = () => {
    try {
      ...
      logoImg.onload = () => {
        ...
        // Draw the transparent logo image directly without color overlay
        ctx.drawImage(logoImg, logoX, logoY, logoSize, logoSize);
        ...
        ctx.font = 'bold 24px TelenorEvolution, sans-serif';
        ctx.textAlign = 'left';
        ctx.fillText('GPStar', logoX + logoSize + 20, logoY + 35);
